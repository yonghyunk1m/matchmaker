...
def main():
    parts = []
    parts.append("# Results Tracking — directional plan / hybrid / audio detector")
    ...
    parts.append(section_header("4. Audio-only cascade detector probe (rule-based, no training)"))
    ...
    parts.append(section_header("5. Decisions"))
    parts.append("- **Directional plan**: not integrated (marginal vs paper's quant claim)")
    parts.append("- **Cost-bias Dixon (PlanAwareDixon)**: catastrophic, not integrated; "
                 "covered by paper §6.1 limitations")
    parts.append("- **Hybrid full pipeline**: in progress; if `min Δ ≥ 0` across 159 pairs, "
                 "we have an `always ≥ baseline` claim for §5/§6")
    parts.append("- **Audio detector**: viable proof of concept; framed as future work in §6.1\n")
