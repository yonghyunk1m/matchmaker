"""Confidence-gated v2 on ALL cascade-likely pieces (full performer counts).
This generates Pass1 + Pass2 trajectories needed for cross-piece section-swap.

Cascade-likely = pieces with quant slow_beats >= 100 OR known cascade.

Output: results/confidence_gated_v2/trajectories/{piece}_{perf}_pass1_v2.csv etc.
"""
import os
import pickle
import sys
...
from run_confidence_gated_v2 import (
    section_boundaries, actual_gamma_per_section, build_blended_plan,
)
...
CASCADE_THRESHOLD = 100  # slow_beats >= this → run pass1+pass2
...
def main():
    ...
    for piece in PIECES:
        pid = piece["id"]
        if pid in {"chopin_ballade1", "chopin_ballade_2", "schubert_impromptu_3", "bach_prelude_848"}:
            # Skip already-done pieces from prior runs
            continue
        ...
        slow_beats = int((gamma_rule < 0.85).sum())
        if slow_beats < CASCADE_THRESHOLD:
            print(f"\n{pid}: slow_beats={slow_beats} < {CASCADE_THRESHOLD} → skip")
            continue
        ...
        print(f"\n{pid}: slow_beats={slow_beats}, sections={len(sections)} → run all perfs")
        ...
        for perf in piece["performers"]:
            ...
            if (OUT_TRAJ / f"{tag2}.csv").exists():
                print(f"  ◇ {tag2} already exists, skip")
                continue
            t0 = time.time()
            try:
    ...
    df.to_csv(OUT_ROOT / "all_cascade_pass2.csv", index=False)
    print(f"\nSaved {len(df)} additional cascade rows")
